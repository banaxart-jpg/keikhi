...
## 実験 12: 厳格 confluence (「判断材料足りなければ買わない」)

**日時**: 2026-06-16
**user 提案**: 「100 箇所ランダムサンプルで判断するより、判断材料足りない時は買わない選択を取れない?」
**スクリプト**: `/tmp/fx_confluence.mjs`
**仮説**: 実験 11 まで 7799 signals/1.4 年 = 1 日 15 回 trade で out-of-shape。
        「勝つ retail の特徴 D: 90% は手出さない」を実装したら edge 出るか?

### 4 フィルタ (全部揃った時だけ撃つ)
1. **MTF EMA20/50** が D1/H4/H1 で 3 つ揃ってる方向
2. **ATR** が 50 本 MA より大きい (= ボラあり)
3. **RSI** が trend-confirming zone (LONG: 50-70、SHORT: 30-50)
4. **時間帯** が London 07-11 UTC or NY 12-16 UTC

各フィルタが方向 (UP/DOWN/NEUTRAL) を投票、N 個揃った時だけ trade。
...
= 「全フィルタ揃った瞬間」は 3.8% (約 月 7-8 時間) しかない。残りは判断材料不足。

### 結果
| minScore | sig/年 | LONG/SHORT | 勝率 | avg net |
|---|---|---|---|---|
| 2/4 (ゆるい) | 4711 | 4000/2596 | 46.7% | -2.7 |
| 3/4 (中) | 1918 | 1733/952 | 47.5% | -2.5 |
| **4/4 (超厳しい)** | **292** | **290/119** | **49.9%** | **-0.7** |
| 4/4 + TP×1.5 | 292 | 290/119 | 39.4% | -1.5 |

### Robustness (4/4 split)
- 前半: 47.0% / -1.2 pips (バランス regime)
- 後半: 53.1% / -0.2 pips (USD/JPY 上昇 trend、ほぼ全 LONG)
- regime fit じゃない (前後半ほぼ同じ avg)

### 数学的解釈
50% × +1 ATR + 50% × -1 ATR = ±0 pips (gross)
-1 pip cost = -0.7 pips net (実測と一致)

= **cost を超える edge が無い**。フィルタ厳格化で:
- signals 19 倍削減 (7799 → 409)
- avg net 3 倍改善 (-2.1 → -0.7)
- 月 trade 数 ~25 (retail で勝つ人の 5-10 件にはまだ多い)

しかし **break-even が天井**。これ以上絞っても勝率は ±2pt しか動かない。

### 結論
**user の直感「PASS 増やす」 = 正しい改善方向**でした。
- 確実に「マシ」にはなる (cost drag が減るから)
- でも edge は生まれない (= 50% wr の壁を超えられない)
- = TA の本質的限界、効率市場仮説の実証

### 残選択肢 (更新)
- F. ML (overfitting リスクと闘う)
- **I. 凍結** ← この時点でほぼ確定
...
## 実験 15: Wemof (わっきゃい式 BB±3σ 平均回帰スキャル) の機械化

**日時**: 2026-07-04
**user 依頼**: 「wemof ってゆう取引方法落とし込めない?」
**実装**: `fx-lib/strategies.js` の `wemof` 戦略 (本実装、fx-bot UI から使用可)
**データ**: Dukascopy USD/JPY M5 bid、2025-07-01 〜 2026-06-25 (99,072 本 ≒ 1 年)
**エンジン**: 本番 backtest engine と同一規約のローカル再現
  (シグナル翌足寄り entry / 同バー両接触 SL 優先 / cost 閾値シフト)

### ルール (機械化)
BB(20,±3σ) 突き抜け + RSI(14) 極値 + 翌足の勢い低下 (実体縮小・高安更新なし) で逆張り。
「説明のつく動きはスキップ」の機械近似: 直近96本高安ブレイク / キリ番±5pips /
バンドウォーク候補 / EMA50 強トレンド / BB幅4pips未満 (閑散) を見送り。

### 結果 (全構成マイナス)
| TP/SL/cost | 勝率 | 期待値/回 | 年間 |
|---|---|---|---|
| 2/6/1.0 (本家想定) | 65.6% | -1.75 pips | -112 pips |
| 3/6/1.0 | 65.6% | -1.09 | -70 |
| 3/3/1.0 | 43.8% | -1.38 | -88 |
| 4/4/1.0 | 51.6% | -0.82 | -52 |
| 2/6/0.4 (低スプ) | 67.2% | -1.02 | -66 |
| 3/3/0.4 | 50.0% | -0.40 | -26 |

シグナル 64 回/年 (LONG51/SHORT13)。

### 発見 (重要)
勝率 65% は TP2/SL6 の壁の非対称性だけで説明がつく。ランダム entry の
理論勝率 = SL/(TP+SL) = **75%** であり、Wemof 機械化版はそれを**下回る**
(3/3 対称でも 43.8% < 50%)。= スパイク後はわずかに継続方向に流れる。

### 結論
機械化 Wemof に edge 無し (実験 1-12 の「retail simple TA に edge 無し」と整合)。
本家の成績の源泉は機械化できない「この動きは不合理か?」の裁量 +
リアルタイムの板/スプレッド/指標感覚と推定。
使うならアラート用途 (±3σ 検知 → 最後の判断だけ人間) を推奨。
注意: bid のみ / 1 instrument-year / 64 trades (勝率 CI ±12%) / セッションフィルタなし。
