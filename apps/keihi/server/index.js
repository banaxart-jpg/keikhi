...
★前提知識ゲート (★★全テキストフィールドに適用★★):
${vocabLine}
- question / options(4個) / explanation / claude_example のすべてで、
  ユーザーが知ってる用語 + 一般日本語 (中学生レベル) + 「${genre}」自体の概念 のみ使う
- 「クライアント」「サーバー」「プロセス」「並行」「スレッド」「キャッシュ」等の
  IT 専門用語は、ユーザーの既知リストに無ければ options に入れるのも禁止
- どうしても他の専門用語が必要なら問題文で1行説明 + prerequisites に列挙
- options を専門用語で書くな。日常語で「待つ / 同時に進める / 分ける / まとめる」のように。
