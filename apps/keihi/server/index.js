...
【出力形式】プレーンテキスト、見出しは下記の【】記号付きで区切る。マークダウン記法は使わない。全体で800〜1200字を目安に。

【決定事項】何をやる / やらないか。1〜3文で断定的に。両論併記禁止。議論で複数の合意点があれば、それらを統合した1つの「やること」として書く。

【根拠】3者の議論で出た事実・出典・統計を根拠として 3〜5点。「(出典: ◯◯)」のような形で議論ログにあった出典を引用する。議論で出ていない事実は使わない。

【実行プラン】時間軸（例: 0〜90日 / 12カ月）と具体的なアクションを 3〜5項目。議論で出た PoC・KPI・組織設計（PMO 等）の提案は最大限拾う。

【KPI】議論で出た数値指標を箇条書きで明示。

【撤退ライン / 見直し基準】どの KPI が未達なら撤退・方針転換するか。議論で明示されていなければ「議論で明示されず」と書く。

【3者の立場の違いと未解決の論点】議論で完全に合意できなかった点を1〜3点。なければ「無し」と書く。

決定的に書くこと。「ケースバイケース」「状況による」「検討が必要」は禁止。実行できる結論にすること。`;
